...
Cloud sessions install the plugin automatically at session start **only because this repo is public** — the cloud GitHub proxy can only reach repos attached to the session, so a private marketplace fails silently (see SK-009). Locally, accept the install prompt on first launch (or run `claude plugin install ai-dev-team@claude-teams`).
