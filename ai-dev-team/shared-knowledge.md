...
## [SK-022] Finding 會生 finding —— 冇 gate 嘅 finding 修完會種出下一條

**日期**：2026-09-17 17:40
**來源 Agent**：Main Agent（Run365Days v3.3.0 retrospective）
**類別**：錯誤模式 / 流程
**適用 Agent**：全部（尤其 Code Reviewer、Quality Assurance、Main Agent）
**有效期至**：永久

**內容**：

Review / QA 捉到嘅嘢唔係全部同一種。**改到行為嗰啲會收斂，純措辭嗰啲唔會。**

實例（Run365Days，同一個 `CLAUDE.md` §6 表格 row，三轉）：

| 輪 | Finding | 修法 | 後果 |
|---|---|---|---|
| 1 | **W-060** —— row 寫「345 → 69 行」，`wc -l` 實測 351 → 75 | 改成百分比 | 種出 W-061 |
| 2 | **W-061** —— 修 W-060 嗰個 commit 自己嘅 docstring 又引咗「573 tests」，今日重量係 577 | 釘死 commit `99abfcc`，實跑 `--collect-only` 核到 573 | 種出 S-128 |
| 3 | **S-128** —— W-060 個修法寫「刻意唔寫絕對行數」之後即刻寫咗四個 | 改措辭做「唔攞嚟做核對基準」 | 收 |

**三轉都係真嘅、都係啱嘅、冇一轉改到軟件行為。** 同族 finding 喺同一個 repo 一共七條
（W-017 / S-068 / S-069 / W-060 / W-061 / S-105 / S-128）。

另一族係簿記：S-125 / S-126 / S-127 全部係 ticket registry 三處 census 數字唔一致 ——
**簿記嘅簿記**。S-125 最難自己發現：「編號連續 / 無跳號 / 無撞號」三句結論喺加票之後照舊成立，
所以段落停留喺 `0001…0042` / 42 張（實際 52 張）讀落一樣順。

**做法**：
1. **Finding 落編號之前先問：呢條收唔收得返？** 改到行為、或者可以用測試／mutant 釘住 → `blocking`，
   開編號開 commit。純措辭／文件用字／簿記數字 → `log-only`，報告寫一段就算。
2. **根治方法係禁止而唔係逐條修**：唔准喺註釋／文件寫冇 gate 嘅絕對數字。要寫就
   (a) 加測試釘住、(b) 寫「喺 `<commit>` 量到 N」、(c) 改成可核實描述。一條規則消滅成個 class。
3. **簿記數字唔准人手維護**，由指令生成（`find … | wc -l`），而且相關幾處一次過改。
4. 判斷一段文件有冇過時，**唔可以靠讀落順唔順** —— 會過時嘅通常係範圍同數字，唔係結論。

**適用場景**：
任何 `/review` `/test` 之後嘅 finding 分流；決定開唔開 W/S 編號；決定一條 finding 值唔值得開 commit；
sprint retrospective 分析「點解做咗咁耐」。

**參考**：
`.proj-docs/reviews/2026-09-17_review_CUI-0047_batch.md`（W-060）、
`.proj-docs/qa/2026-09-17_qa_v3.3.0.md`（W-061、S-125…S-128）、
`rules/global-rules.md` §Review / QA 經濟學
