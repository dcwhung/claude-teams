---
description: DevOps 執行 CI/CD 部署流程
...
- Pipeline 各階段、環境變數管理、回滾流程 → `skills/sw-ci-cd/SKILL.md`
- 部署記錄格式 → `agents/devops-engineer.md`
- 部署完成後 handoff（smoke test、回滾判斷）→ `skills/sw-post-review-handoff/SKILL.md` → Protocol 4
- develop → main merge 規則 → `skills/sw-git-flow/SKILL.md`
...
> 實際回滾指令模板見 `skills/sw-ci-cd/SKILL.md` → 回滾流程。
