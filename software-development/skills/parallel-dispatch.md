...
> 與 `skills/autonomous-loop.md`「模式二：Parallel Agents」嘅關係：
...
- 低層 dispatch 機制：`skills/autonomous-loop.md` 模式二（Parallel Agents）
- 單 fix 流程：`commands/fix.md` / `feature.md` / `refactor.md`
- Branch / commit 規範：`skills/git-flow.md`
- Reviewer batch 行為：`agents/code-reviewer.md` § Batch Review
- QA batch 行為：`agents/quality-assurance.md` § Batch QA
- Receipt 格式：`skills/post-review-handoff.md`
- Compact protection：`skills/agent-protocols.md` §10
